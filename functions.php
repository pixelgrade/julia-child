<?php
/**
 * Julia Child functions and definitions
 *
...
/**
 * Setup Julia Child Theme's textdomain.
 *
 * Declare textdomain for this child theme.
 * Translations can be filed in the /languages/ directory.
 */
function julia_child_theme_setup() {
	load_child_theme_textdomain( 'julia-child-theme', get_stylesheet_directory() . '/languages' );
}
add_action( 'after_setup_theme', 'julia_child_theme_setup' );
...
function julia_child_enqueue_styles() {
	...
	if ( !is_rtl() ) {
		wp_enqueue_style( 'julia-style', get_template_directory_uri() . '/style.css', array(), $parent->get( 'Version' ) );
	} else {
		wp_enqueue_style( 'julia-style', get_template_directory_uri() . '/rtl.css', array(), $parent->get( 'Version' ) );
	}
	// Here we are adding the child style.css while still retaining
	// all of the parents assets (style.css, JS files, etc)
	wp_enqueue_style( 'julia-child-style',
		get_stylesheet_directory_uri() . '/style.css',
		array('julia-style') //make sure the the child's style.css comes after the parents so you can overwrite rules
	);

}

add_action( 'wp_enqueue_scripts', 'julia_child_enqueue_styles' );
...
function julia_child_overwrite_files() {
	...
	// See the parent's function.php -> the julia_scripts_styles() function
	// for details like resources names
	//
	// Remember that the rest of the static resources will still get loaded like 
	// julia-imagesloaded, julia-hoverintent and julia-velocity

		wp_dequeue_script( 'julia-scripts' );
	...
	// This is not required, but I assume you are not modifying that much :)
		...
		wp_enqueue_script( 'julia-child-scripts',
			get_stylesheet_directory_uri() . '/assets/js/main.js',
			array( 'jquery', 'masonry', 'julia-imagesloaded', 'julia-hoverintent', 'julia-velocity' ),
			'1.0.0', true );
	...
	// see the parent's function.php -> the julia_scripts_styles() function for details like resources names
	
		wp_dequeue_style( 'julia-style' );
	...
	// Now you can add your own, modified version of the "style.css" file
		...
		wp_enqueue_style( 'julia-child-style',
			get_stylesheet_directory_uri() . '/style.css',
			array('julia-font-awesome-style') //use the same dependencies as the parent because we want to still use FontAwesome icons
		);
}

// Load the files from the function mentioned above:

	add_action( 'wp_enqueue_scripts', 'julia_child_overwrite_files', 11 );
